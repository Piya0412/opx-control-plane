# Historical Incident Pattern Agent - Prompt Changelog
...
#### Added
- Complete system instructions for historical pattern analysis
...
- Analysis framework (similarity assessment, pattern recognition, hypothesis formation)
...
**Why read-only projections?**
- Agents must not query live incident tables (control plane authority)
- Projections are frozen snapshots, ensuring replay determinism
- Prevents race conditions and data consistency issues

**Why pattern-based confidence?**
- Historical patterns provide statistical confidence
- Success rate and recency directly impact reliability
- Multiple similar incidents increase confidence

**Why no execution recommendations?**
- Agents provide hypotheses, not decisions
- Phase 5 (automation) is the sole executor
- Human approval required for all actions

**Why similarity scoring?**
- Service name, severity, and symptoms determine relevance
- Quantifiable similarity enables confidence calculation
- Explicit similarity thresholds prevent false positives

#### Alignment with Contracts

- **Input:** Matches `HistoricalIncidentInput` from `docs/AGENT_CONTRACTS.md`
- **Output:** Matches `AgentOutput` envelope with `HistoricalPatternFindings`
...
4. Enforced hypothesis-only framing
