...
def test_environment_variables() -> bool:
    ...
    Returns:
        True if all environment variables are set (or if config has them)
    """
    ...
    for var in required_vars:
        ...
        if value:
            ...
            print_info(f"{var} not set (will use bedrock_config.py)")
            all_set = False
    
    if all_set:
        print_success("All environment variables set")
    else:
        print_info("Some environment variables missing")
        print_info("Note: Graph will use bedrock_config.py as fallback")
        print_info("Optional: Run 'source src/langgraph/set_agent_env.sh' to set them")
        # Don't fail the test - config fallback works
        return True
...
def test_single_agent_invocation() -> bool:
    ...
    try:
        ...
        # Disable DynamoDB checkpointing for testing
        os.environ['USE_DYNAMODB_CHECKPOINTING'] = 'false'
        
        # Recreate graph with updated env var
